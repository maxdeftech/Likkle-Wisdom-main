...
const PWAInstallPrompt: React.FC<PWAInstallPromptProps> = ({ install }) => {
    const {
        canInstall,
        isIOS,
        isStandalone,
    ...
    if (!canInstall || (!showPrompt && !showIOSInstructions && !isMinimized)) return null;

    if (isMinimized) {
        return (
            <button
                onClick={handleMaximize}
                className="fixed bottom-24 right-4 z-[9999] size-14 rounded-full glass bg-white/10 border border-white/20 shadow-2xl flex items-center justify-center animate-bounce-slow lg:hidden"
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in lg:hidden">
